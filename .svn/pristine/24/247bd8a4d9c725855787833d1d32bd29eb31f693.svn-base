...
export default {
  ...
  methods: {
      ...
      this.$http
        ...
        .then((res) => {
          ...
          data.forEach((item, index) => {
            if (item.checkCate === "899233FA-CFC1-4D68-B2EF-EFE676FF41A6") {
    ...
    getPoNo() {
      this.$http
        .get("/api/whseYarninDtl/v1.0/distinct?distinctStr=batch_No")
        .then((res) => {
    ...
    fn_PostTye() {
      this.$http
        .get("/api/qcCheckCate?cateId=899233FA-CFC1-4D68-B2EF-EFE676FF41A6")
        .then((res) => {
