...
export default {
  ...
  methods: {
      ...
      this.$http
        ...
        .then((res) => {
          ...
          data.forEach((item, index) => {
            if (item.checkCate === "C55F4019-62C8-4F7F-B5B4-C19F316A5982") {
    ...
    getPoNo() {
      this.$http
        .get("/api/whseCalicoinDtla/v1.0/distinct?distinctStr=batch_No")
        .then((res) => {
          let resData = res.data;
          this.sortNum(resData, "seqty");
          let poData = [];
          if (resData.length === 0) {
            return;
          }
          resData.forEach((item, index) => {
            let data = {
              label: item.batchNo,
              value: item.whseCalicoinDtlaoid,
            };
            poData.push(data);
          });
    ...
    fn_PostTye() {
      this.$http
        .get("/api/qcCheckCate?cateId=C55F4019-62C8-4F7F-B5B4-C19F316A5982")
        .then((res) => {
