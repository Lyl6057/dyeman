...
export default {
  ...
  methods: {
    ...
    del() {
      ...
      this.$tip
        .cofirm(
          "是否确定删除編號為 【" + this.chooseData.materialNum + " 】的數據？",
          this,
          {}
        )
        .then(() => {
    ...
    save() {
      ...
      this.saved = true;
      if (this.form.whseAccessoriesinoid) {
        updateScfl(this.form).then((Res) => {
          if (this.mx.length === 0) {
            this.loading = false;
            ...
            return new Promise((resolve, reject) => {
              ...
              this.mx.forEach((item, i) => {
                if (item.whseAccessoriesDtloid) {
                  // 更新
                  updateScflDetali(data).then((res) => {
                    resolve();
                  });
                } else {
                  // 新增
                  data.whseAccessoriesinFk = this.form.whseAccessoriesinoid;
                  addScflDetali(data).then((res) => {
            ...
            for (let i = 0; i < this.mx.length; i++) {
              if (this.mx[i].loc && this.mx[i].loc.length > 0) {
                this.mx[i].loc.forEach((item) => {
                  ...
                  if (!item.whseAccessoriesDtlaoid) {
    ...
    choiceData(val) {
      ...
      if (this.choiceTle === "選擇來辅料登記明細") {
        val.forEach((item, i) => {
          item.poQty = item.incomQty;
          item.unitQty = item.qtyUnit;
          item.materialNum = item.$basProductivesuppliesFk;
          item.materialName = item.$basProductivesuppliesFk;
        });
        this.mx = this.mx.concat(val);
        // this.mx = this.$unique(this.mx.concat(val), "batchNo");
        this.mx.forEach((item, i) => {
